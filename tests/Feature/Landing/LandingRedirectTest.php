...
class LandingRedirectTest extends TestCase
{
    ...
    /**
     * The redirect is TEMPORARY -- landing_page_redirects.expires_at is
     * now + 90 days -- but 301 is the status browsers cache hardest, and
     * Symfony deliberately strips the default no-cache from a 301
     * (RedirectResponse::__construct), so without an explicit directive the
     * response ships with no freshness information at all and a browser is
     * free to treat it as permanent. Confirmed in real Chromium: after the
     * server stopped serving the redirect the browser still followed it
     * without revalidating, and a rename-back (a -> b -> a, which the admin
     * update() explicitly supports) produced ERR_TOO_MANY_REDIRECTS on the
     * tenant's own live URL -- a state no server-side change can undo,
     * because the server is never asked. The redirect therefore carries the
     * same short, revalidating freshness the page itself does.
     */
    public function test_the_redirect_is_cacheable_only_as_briefly_as_the_page_itself(): void
    {
        $page = $this->makePage('renamed-to');
        $this->insertRedirect('renamed-from', $page->id, now()->addDays(30));

        $res = $this->get($this->url('renamed-from'));

        $res->assertStatus(301);

        $cacheControl = $res->headers->get('Cache-Control') ?? '';

        // Directive-wise rather than string-wise: Symfony re-serialises
        // Cache-Control in its own (alphabetical) order, so asserting the
        // literal string would test Symfony's formatter, not the policy.
        $this->assertSame(
            (int) config('landing.cache_ttl'),
            (int) $res->headers->getCacheControlDirective('max-age'),
            'The 301 carries no max-age, so a browser may cache a 90-day redirect forever: ' . $cacheControl
        );
        $this->assertTrue(
            $res->headers->hasCacheControlDirective('must-revalidate'),
            'The 301 must revalidate once stale: ' . $cacheControl
        );
        $this->assertTrue(
            $res->headers->hasCacheControlDirective('public'),
            'The 301 is public content and shared caches may hold it: ' . $cacheControl
        );
    }
