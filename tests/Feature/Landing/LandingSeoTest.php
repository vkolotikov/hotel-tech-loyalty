...
class LandingSeoTest extends TestCase
{
    ...
    /**
     * Four ratings is enough for reviewStats, but the VISIBLE band needs a
     * featured review with a comment — and nothing in the product can set
     * is_featured today, so "rated, none featured" is not an edge case, it
     * is the default state of every real tenant. Publishing aggregateRating
     * for a page that renders no reviews at all is invisible structured
     * data, which Google's structured data policies forbid outright and
     * which risks a manual action against the tenant's whole site. So the
     * markup follows the BAND, not the rating count.
     *
     * Tested in both directions -- band absent below, band present in the
     * sibling test -- because a one-directional assertion here is satisfied
     * by deleting the markup outright.
     *
     * review_submissions carries no brand_id, so reviews are ORG-wide and
     * the two directions cannot share a fixture: any featured review created
     * for the positive case would be visible to the negative one too. Hence
     * two tests rather than two pages in one.
     */
    public function test_no_aggregate_rating_is_published_when_the_reviews_band_is_absent(): void
    {
        $page = $this->published(19, 'band-hidden');
        Property::create([
            'organization_id' => 1, 'brand_id' => 19, 'name' => 'Hidden Co', 'is_active' => true,
        ]);

        // Enough ratings for reviewStats (4), none of them featured -- which
        // is every real tenant, since no product surface sets is_featured.
        foreach ([5, 4, 5, 3] as $i => $rating) {
            ReviewSubmission::create([
                'organization_id' => 1, 'overall_rating' => $rating,
                'comment' => "Unfeatured {$i}", 'is_featured' => false, 'submitted_at' => now()->subMinutes($i),
            ]);
        }
        ...
        $this->assertArrayNotHasKey(
            'aggregateRating',
            $data,
            'The page publishes an aggregateRating for reviews a visitor cannot see anywhere on it.'
        );
        $this->assertArrayNotHasKey('review', $data);
    }

    /** The other direction: band rendered, so the markup describes it. */
    public function test_the_aggregate_rating_is_published_when_the_reviews_band_renders(): void
    {
        $page = $this->published(20, 'band-shown');
        Property::create([
            'organization_id' => 1, 'brand_id' => 20, 'name' => 'Shown Co', 'is_active' => true,
        ]);

        foreach ([5, 4, 5, 3] as $i => $rating) {
            ReviewSubmission::create([
                'organization_id' => 1, 'overall_rating' => $rating,
                'comment' => "Featured {$i}", 'is_featured' => true, 'submitted_at' => now()->subMinutes($i),
            ]);
        }
    ...
    /**
     * Four featured, rated reviews: enough for reviewStats AND for
     * has('reviews'), so the only thing left deciding whether the band
     * renders is the section row itself.
     */
    private function fourFeaturedReviews(): void
    {
        foreach ([5, 4, 5, 3] as $i => $rating) {
            ReviewSubmission::create([
                'organization_id' => 1, 'overall_rating' => $rating,
                'comment' => "Featured {$i}", 'is_featured' => true,
                'submitted_at' => now()->subMinutes($i),
            ]);
        }
    }
    ...
    /**
     * @dataProvider bandSuppressedByItsSectionRow
     *
     * reviewStats and has('reviews') are not the whole gate, and gating the
     * markup on them was the same defect through a different door. The
     * section loop skips a row whose `enabled` is false, and a page can
     * simply have no `reviews` row at all — a template rollback, a section
     * set written before `reviews` existed, or a tenant who switched the
     * band off. In both states the tenant has four featured, rated reviews,
     * so both of those switches say yes, and the page still renders no band
     * anywhere. Publishing an aggregateRating over that is invisible
     * structured data, which Google's structured data policies forbid
     * outright and which risks a manual action against the tenant's whole
     * site.
     *
     * The JSON-LD block itself must SURVIVE — a page is not fixed by losing
     * its LocalBusiness node — so the business name is asserted out of the
     * decoded payload rather than the absences alone.
     */
    public function test_no_aggregate_rating_is_published_when_the_band_is_switched_off(int $brandId, string $slug, string $how): void
    {
        $page = $this->published($brandId, $slug);
        Property::create([
            'organization_id' => 1, 'brand_id' => $brandId, 'name' => 'Off Co', 'is_active' => true,
        ]);
        $this->fourFeaturedReviews();

        match ($how) {
            'disabled' => $page->sections()->where('key', 'reviews')->update(['enabled' => false]),
            'missing'  => $page->sections()->where('key', 'reviews')->delete(),
        };
        ...
        $this->assertSame('Off Co', $data['name'],
            'The whole structured-data block was dropped, not just the review markup.');

        $this->assertArrayNotHasKey(
            'aggregateRating',
            $data,
            "The page publishes an aggregateRating for reviews a visitor cannot see anywhere on it (section row {$how}).",
        );
        $this->assertArrayNotHasKey('review', $data);
    }

    /** @return array<string, array{0: int, 1: string, 2: string}> */
    public static function bandSuppressedByItsSectionRow(): array
    {
        return [
            'the tenant switched the band off' => [21, 'band-disabled', 'disabled'],
            'the page has no reviews row'      => [22, 'band-missing',  'missing'],
        ];
    }
