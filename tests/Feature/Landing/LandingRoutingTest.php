...
class LandingRoutingTest extends TestCase
{
    ...
    // ─── Preview: the only route here that serves an UNPUBLISHED draft ───

    /**
     * `preview` calls LandingPage::withoutGlobalScopes()->find($page): no
     * tenant scope, no `published()` filter, no authentication. Its ENTIRE
     * protection is the one chained `->middleware('signed')` in
     * routes/landing.php — and nothing in this suite touched it, so deleting
     * that single call left every test green while /preview/1, /preview/2, …
     * served every tenant's draft to any anonymous visitor who counted.
     *
     * The body assertion is not belt and braces: it is what makes this test
     * about the draft staying private rather than about a status code, so a
     * future refactor that leaks the draft under some other status still
     * goes red.
     */
    public function test_an_unsigned_preview_request_is_refused(): void
    {
        $page = $this->draft('Unannounced Spring Menu');

        $response = $this->get('http://' . $this->host() . '/preview/' . $page->id);

        $response->assertForbidden();
        $this->assertStringNotContainsString('Unannounced Spring Menu', $this->body($response),
            'An unsigned request read an unpublished draft.');
    }

    /**
     * The other half, and not a formality. On its own the test above passes
     * just as well against a route that refuses EVERYBODY — `signed` swapped
     * for a deny-all, or the route deleted outright — while preview was dead
     * for the one person it exists to serve. Neither test can go vacuous
     * while both are here.
     *
     * On the signature and tenancy: the signed URL is unguessable but it is
     * NOT tenant-bound — nothing in preview() checks who the page belongs to.
     * That is safe only because no tenant can obtain another tenant's URL:
     * previewUrl() takes no page id from the request and reads the caller's
     * own page through the tenant scope, and the signature covers the id, so
     * editing it invalidates the URL (LandingPageAdminApiTest asserts that).
     */
    public function test_a_validly_signed_preview_url_still_renders_the_draft(): void
    {
        $page = $this->draft('Unannounced Spring Menu');
        ...
        $response->assertOk();
        $this->assertStringContainsString('Unannounced Spring Menu', $this->body($response),
            'A valid signed preview URL did not show the tenant their own draft.');
    }

    /** A draft with something on it, so "did the draft leak?" is answerable. */
    private function draft(string $headline): LandingPage
    {
        $page = $this->make('not-ready', 'draft');
        $page->update(['content' => ['hero' => ['headline' => $headline]]]);
