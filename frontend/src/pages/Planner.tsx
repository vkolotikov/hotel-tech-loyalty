...
export function Planner() {
  ...
  const { data: tasks = [] } = useQuery({
    queryKey: ['planner-tasks', tab, tab === 'day' ? currentDate : tab === 'week' ? weekStart : monthYear, employee],
  ...
  // Task card component
  const TaskCard = ({ task, dateStr }: { task: any; dateStr: string }) => (
    <div className={'p-2 rounded-lg bg-dark-surface2 border-l-2 ' + (PRIORITY_COLORS[task.priority] ?? 'border-l-gray-600') + ' text-xs'}>
        ...
        <>
          ...
          <div className="bg-dark-surface border border-dark-border rounded-xl p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => openCreate(currentDate)} className="flex items-center gap-1.5 text-xs text-primary-400 hover:text-primary-300">
      ...
      {showCreate && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-dark-surface border border-dark-border rounded-2xl w-full max-w-md p-6">
            <h2 className="text-base font-semibold text-white mb-4">Add Task &mdash; {createDate}</h2>
