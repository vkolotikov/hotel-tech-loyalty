...
import { ArrowLeft, RefreshCw, Send, User, Calendar, MapPin, DollarSign, MessageSquare } from 'lucide-react'

const STATUS_OPTIONS = ['new', 'confirmed', 'checked-in', 'checked-out', 'cancelled', 'no-show']
const INVOICE_OPTIONS = ['none', 'draft', 'sent', 'paid', 'overdue']
            ...
            <div className="grid grid-cols-2 gap-6">
              <div>
                ...
                <div className="space-y-2">
                  ...
                  {b.guest && (
                    <Link to={`/guests/${b.guest.id}`} className="text-xs text-primary-400 hover:text-primary-300">
                      View CRM Profile →
                    </Link>
                  )}
                </div>
          ...
          <div className="bg-dark-800 rounded-xl border border-dark-700 p-5">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div>
                ...
                <div className="text-sm text-white">{b.deposit_amount ? `€${b.deposit_amount}` : '—'} {b.deposit_paid ? '✓' : ''}</div>
              </div>
              <div>
                <div className="text-xs text-gray-500">Prepayment</div>
                <div className="text-sm text-white">{b.prepayment_amount ? `€${b.prepayment_amount}` : '—'} {b.prepayment_paid ? '✓' : ''}</div>
              </div>
            </div>
            ...
            <div className="space-y-2 text-sm">
              ...
              {b.notice && (
                <div className="pt-2 border-t border-dark-700">
                  <div className="text-gray-500 text-xs">Guest Notes</div>
                  <p className="text-gray-300 text-xs mt-1">{b.notice}</p>
                </div>
              )}
              {b.synced_at && (
