...
import {
  FileText, Package, FilePlus2, TrendingUp,
  GitBranch, Sparkles, ArrowLeft, ChevronRight, Search,
} from 'lucide-react'

/**
 * "Pipeline" hub — the "what's happening" half of the CRM. Mirrors the
 * grid-home + breadcrumb-leaf pattern used by Settings and Chatbot
 * Setup so the whole admin app reads as one coherent navigation
 * system.
 *
...
type TabKey = 'inquiries' | 'deals' | 'insights' | 'lead-forms'
...
const TABS: TabDef[] = [
  ...
  { key: 'deals',      label: 'Deals',             desc: 'Won deals working through fulfillment, invoicing, and stay',  icon: Package },
  { key: 'insights',   label: 'Insights',          desc: "Today's snapshot, going-cold deals, stuck stages, value map", icon: TrendingUp },
...
const SECTIONS: Section[] = [
  {
    id: 'pipeline',
    label: 'Pipeline',
    desc: 'The records — leads moving through the funnel and deals you have closed',
    icon: GitBranch,
    accent: '#c9a84c', // gold
    tabs: ['inquiries', 'deals'],
  },
  {
    id: 'setup',
    label: 'Setup & Insights',
    desc: 'How new leads come in and how the pipeline is performing',
    icon: Sparkles,
    accent: '#a78bfa', // violet
    tabs: ['lead-forms', 'insights'],
  },
]
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-t-secondary mt-0.5">Where leads enter, how they progress, and how they close.</p>
      </div>
      ...
      {onHome ? (() => {
        ...
        const filteredSections = SECTIONS
          .map(s => ({
            ...
            visibleTabs: s.tabs
              ...
              .filter(td => {
                if (!q) return true
                return td.label.toLowerCase().includes(q)
        ...
        <div className="space-y-5">
          ...
          <Suspense fallback={fallback}>
            ...
            {active === 'deals'      && <Deals />}
            {active === 'insights'   && <InquiryInsights />}
