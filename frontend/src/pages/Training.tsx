    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-[#8e8e93]">Train custom AI models using your knowledge base data</p>
        </div>
      </div>
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-dark-surface border border-dark-border rounded-xl p-4">
            <div className="flex items-center gap-2 text-[#8e8e93] text-xs mb-1"><Database size={14} /> Active FAQ Items</div>
            <div className="text-2xl font-bold text-white">{stats.faq_count}</div>
          </div>
          <div className="bg-dark-surface border border-dark-border rounded-xl p-4">
            <div className="flex items-center gap-2 text-[#8e8e93] text-xs mb-1"><FileText size={14} /> Processed Documents</div>
            <div className="text-2xl font-bold text-white">{stats.document_count}</div>
          </div>
          <div className="bg-dark-surface border border-dark-border rounded-xl p-4">
            <div className="flex items-center gap-2 text-[#8e8e93] text-xs mb-1"><GraduationCap size={14} /> Training Jobs</div>
            <div className="text-2xl font-bold text-white">{stats.jobs_count}</div>
          </div>
          <div className="bg-dark-surface border border-dark-border rounded-xl p-4">
            <div className="flex items-center gap-2 text-[#8e8e93] text-xs mb-1"><RefreshCw size={14} /> Active Jobs</div>
      ...
      <div className="flex items-center gap-3">
        <button onClick={() => exportData.mutate()} disabled={exportData.isPending}
          className="flex items-center gap-2 bg-dark-surface border border-dark-border text-white px-4 py-2 rounded-lg hover:bg-[#222] text-sm disabled:opacity-50">
      ...
      {showCreate && (
        <div className="bg-dark-surface border border-dark-border rounded-xl p-6 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setShowCreate(false)} className="text-[#8e8e93] hover:text-white"><XCircle size={18} /></button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm text-[#8e8e93] mb-1">Base Model</label>
            ...
            <div>
              <label className="block text-sm text-[#8e8e93] mb-1">Epochs</label>
              <input type="number" min={1} max={50} value={epochs} onChange={e => setEpochs(Number(e.target.value))}
                className="w-full bg-[#1c1c1e] border border-dark-border rounded-lg px-3 py-2 text-white text-sm" />
              <p className="text-xs text-[#555] mt-1">How many times to iterate over the training data (3 is typical)</p>
            </div>
          </div>

          <div className="flex justify-end gap-2">
            <button onClick={() => setShowCreate(false)} className="px-4 py-2 text-sm text-[#8e8e93] hover:text-white">Cancel</button>
            <button
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-xs text-[#8e8e93]">{job.base_model}</span>
                    </div>
                    <div className="text-xs text-[#555] space-x-3">
                    ...
                    {job.result_metrics && Object.keys(job.result_metrics).length > 0 && (
                      <div className="flex gap-3 mt-1">
                        {Object.entries(job.result_metrics).map(([k, v]: [string, any]) => (
                          <span key={k} className="text-xs text-[#8e8e93]">{k}: <span className="text-white">{typeof v === 'number' ? v.toFixed(4) : v}</span></span>
                        ))}
                      </div>
                    )}
                  </div>
