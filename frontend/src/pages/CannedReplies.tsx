...
export function CannedReplies() {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setDirty(false)
      toast.success('Canned replies saved')
    },
  ...
  const add = () => {
    if (items.length >= 50) {
      toast.error('Maximum 50 canned replies')
      return
    }
  ...
  const save = () => {
    ...
    if (cleaned.length !== items.length) {
      toast.error('Empty entries will be removed')
    }
    saveMutation.mutate(cleaned)
  }

  if (isLoading) {
    return <div className="text-center text-[#636366] py-12">Loading...</div>
  }
    ...
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-t-secondary mt-0.5">
            Pre-written responses agents can insert with one click from the inbox. Org-wide, max 50.
          </p>
        </div>
      ...
      {items.length === 0 ? (
        <div className="text-center text-[#636366] py-12 border border-dashed border-dark-border rounded-lg">
          No canned replies yet. Click "Add" to create one.
        </div>
      ) : (
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 space-y-2">
                  ...
                  <textarea
                    ...
                    maxLength={2000}
                    rows={3}
                    placeholder="Reply text — what gets inserted into the message box"
      ...
      {dirty && (
        <div className="text-xs text-amber-400">You have unsaved changes.</div>
      )}
    </div>
  )
}
