{{--
  ...
  from resources/landing-kits/gym-tech/02-foundry-strength/index.html; the
  only thing that changed is where the WORDS come from. Read that file
  ...
  contains a raw echo and FoundryStrengthRenderTest asserts that by scanning
  the files — a raw echo here is how a landing page becomes stored XSS on a
  public marketing origin. (The test greps for the opening delimiter, so
  this comment cannot spell it out.)
  ...
  simply not execute. The template's behaviour is public/landing/kit.js,
  external and same-origin — one file shared by every kit template. The one
  <script> with no src is the application/ld+json block, which carries a
  ...
  refusals the seven kit templates before it make:
    ...
    - NO FONT PAIRING. The kit names Bodoni Moda and Manrope in its own
      tokens, both self-hosted (see foundry_strength.css).
    - NO SECTION TONES. This kit alternates black / charcoal / bone / bronze /
      smoke as a designed rhythm, and a band on the wrong surface breaks the
  ...
  offers one (see LandingOnboardingService::rendersFor).
--}}
@php
    use App\Landing\SectionType;
    use App\Support\AssetVersion;

    // THE KIT'S OWN PAGE BACKGROUND (--black in foundry_strength.css),
    // spelled here because App\Support\Accent has to know what surface the
    // tenant's colour will actually be painted on. This page is near-black,
    // so a hex too dark to read as a block on it is lifted toward white —
    // the opposite direction from aera_reformer's ivory.
    ...
    $sectionViews = $sections
        ->mapWithKeys(fn ($section) => [$section->key => SectionType::viewFor($section->key, 'foundry_strength')]);
    ...
    // brand and the monogram box. It stops before config('app.name')
    // deliberately: a nav or a footer headlining US as the business on a
    // club's own site is the mistake every layout here refuses by name.
    $brandName = collect([
    ...
    // wordmark — "Strength club · Riga" — in TWO places (the header and the
    ...
    // because it is the label the four CHROME controls carry as well as the
    // closing panel's own; the hero words its own through `hero.cta_label`.
    //
    // This author writes "Book assessment" on every chrome control, "Book
    // your assessment" in the hero and "Choose a time" on the closing panel.
    // The chrome falls back to his word per placement; the closing panel
    // falls back to the industry's verb, as every kit's does.
    ...
    $chromeLabels    = [
        'header' => 'Book assessment',
        'mobile' => 'Book assessment',
        'footer' => 'Book assessment',
        'fab'    => 'Book assessment',
    ];
...
{{-- The kit's <head> carried a fonts.googleapis.com preconnect pair and a
     ...
     all three tags are gone and the two families are declared as
     self-hosted @font-face rules at the top of foundry_strength.css.
...
<link rel="stylesheet" href="{{ asset('landing/foundry_strength.css') }}{{ AssetVersion::query('landing/foundry_strength.css') }}">
{{-- THE ONE TENANT OVERRIDE, and the only inline CSS on this page (gym-21).

     This kit keeps its accent in THREE tokens: `--bronze`, a FILL — the
     buttons, the offer bar and the whole member-note band — that carries the
     author's BLACK text; `--bronze-soft`, accent TEXT on the dark page (the
     eyebrows, the footer labels, the hero's <em>, the button hover); and
     `--bronze-halo`, the pulse's glow.

     A fill that carries black text has to be light enough for black to
     read on, which is exactly what Accent's `bright` is — the shade measured
     readable on the fixed ink band — so the fill takes `bright`; the soft
     bronze, text on this page's own black, takes `deep`, which on a dark
     surface is the brand lifted until it clears that surface; and the halo
     takes the translucent `halo`. The black, the charcoal, the smoke and the
     bone are SURFACES and stay the author's: repainting a page's surfaces
     ...
     is false). The author's own bronze then stands exactly as he drew it,
     and the page ships zero inline CSS. Every value here is emitted by
     Accent, which routes through CssColor::safe and formats the result
     itself, so none of it is a customer string. --}}
@if ($accent->isDerived)
<style nonce="{{ $cspNonce }}">
  :root{
    --bronze: {{ $accent->bright }};
    --bronze-soft: {{ $accent->deep }};
    --bronze-halo: {{ $accent->halo }};
  }
</style>
@endif
</head>
...
@if ($showsBlock('announcement'))
  @include('landing.foundry_strength.sections.announcement', [
    'copy' => $page->content['announcement'] ?? [],
  ])
@endif
  @include('landing.foundry_strength.header')
  <main id="main">
@if ($trustFirst)
    @include('landing.foundry_strength.sections.trust', ['copy' => $page->content['trust'] ?? []])
@endif
...
@if ($faqBefore === $section->key)
    @include('landing.foundry_strength.sections.faq', ['copy' => $page->content['faq'] ?? []])
@endif
...
@endif
  ...
  @include('landing.foundry_strength.sections.footer', [
