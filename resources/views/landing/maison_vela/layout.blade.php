{{--
  ...
  resources/landing-kits/hospitality/01-maison-vela/index.html; the only thing
  that changed is where the WORDS come from. Read that file beside this one
  before editing either.

  THE FIRST OF THE THREE HOSPITALITY KITS, and the block contract is the
  ...
  contains a raw echo and MaisonVelaRenderTest asserts that by scanning the
  ...
  refusals every kit template makes:
    ...
    - NO FONT PAIRING. The kit names Playfair Display and DM Sans in its own
      tokens, both self-hosted (see maison_vela.css).
    - NO SECTION TONES. This kit alternates paper / paper-deep / ink / olive /
      wine as a designed rhythm, and a band on the wrong surface breaks the
...
@php
    ...
    // THE KIT'S OWN PAGE BACKGROUND (--color-paper in maison_vela.css),
    ...
    // ivory, so a hex too pale to read as a block on it is darkened toward
    // black.
    ...
    $sectionViews = $sections
        ->mapWithKeys(fn ($section) => [$section->key => SectionType::viewFor($section->key, 'maison_vela')]);
    ...
    // wordmark — "Grand Brasserie · Riga" — in TWO places (the header and the
    ...
    // author's wording in four of his five controls — so on a restaurant page
    // the closing panel's default IS his string. Only his header bar differs
    ...
    // transcribed from resources/landing-kits/hospitality/01-maison-vela/index.html, rather
...
{{-- The kit's <head> carried a fonts.googleapis.com preconnect pair and a
     css2 stylesheet <link> for Playfair Display and DM Sans. Both hosts are
     ...
     @font-face rules at the top of maison_vela.css instead — INCLUDING
     Playfair's italic axis, which is the whole of this design's two-tone
     heading treatment.
...
<link rel="stylesheet" href="{{ asset('landing/maison_vela.css') }}{{ AssetVersion::query('landing/maison_vela.css') }}">
{{-- THE ONE TENANT OVERRIDE, and the only inline CSS on this page.
     ...
     tenant's colour.
       ...
       --color-brass       the accent TEXT, and the only place the author
                           paints one: the eyebrow over his kitchen story,
                           every footer label, the fixed pill's hairline. It
                           appears on --color-ink and nowhere else, which is
                           exactly the fixed dark Accent::bright is measured
                           against.
       --color-accent-ink  the same accent on a PAPER ground — the emphasised
                           fragment of every display heading outside the hero.
                           It is introduced by this template's own appended
                           block (the author sets an em in his h1 alone) and
                           defaults to his oxblood, and Accent::deep is
                           measured to 5.5:1 against this kit's own ivory.

     WHAT IS NOT TOUCHED: `--color-wine` and `--color-ink`. Wine is a SURFACE
     here — the announcement bar and the closing panel are painted in it and
     set their type in --color-white and --color-paper-deep, neither of which
     one token can carry with it, so a mid-tone tenant hue would leave a bar
     of unreadable text. Ink is this page's ink. Repainting a page's ink with
     a brand colour is exactly the destruction D2 names, and no kit template
     does it.
...
@if ($showsBlock('announcement'))
  @include('landing.maison_vela.sections.announcement', [
    'copy' => $page->content['announcement'] ?? [],
  ])
@endif

  @include('landing.maison_vela.header')

  <main id="main">
@if ($trustFirst)
    @include('landing.maison_vela.sections.trust', ['copy' => $page->content['trust'] ?? []])
@endif
...
@if ($faqBefore === $section->key)
    @include('landing.maison_vela.sections.faq', ['copy' => $page->content['faq'] ?? []])
@endif
...
@endif
  ...
  @include('landing.maison_vela.sections.footer', [
...
{{-- The persistent Reserve pill, bottom-LEFT. The kit fixes it there and
     ...
     retires it below 42rem so it never covers a phone's content. Rendered
