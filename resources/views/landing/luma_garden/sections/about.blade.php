{{--
  The kitchen (data-block="story", data-variant="ingredient-pairing").

  The author's sage band, split down the middle: on one side a foam eyebrow, a
  display heading, a lead paragraph and a ruled list of short lines; on the
  other a tall rounded photograph.
  ...
  THE LIST IS THE ASIDE FAMILY, drawn as his plain ruled <ul>. `about.note_1..3`
  are the lines and they are the SAME leaves kits 02 and 03-beauty use for the
  same shape; this author writes no label above them, so `about.note_label` is
...
@php
    ...
    // The ruled list. Each leaf SPELLED at its call site rather than passed as
    // a name: `content_fields` is derived by reading this file for the leaves
    // it consumes, so a leaf read through a variable is a leaf the editor would
    // stop offering.
    $noteLines = collect([$copy['note_1'] ?? null, $copy['note_2'] ?? null, $copy['note_3'] ?? null])
        ...
        ->values();
@endphp
    <section @class(['story', 'story--solo' => $storyImage === null]) id="about" data-block="story" data-variant="ingredient-pairing">
