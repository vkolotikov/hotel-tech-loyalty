{{--
  A tenant-added words band (data-block="text", data-variant="garden-note").
  ...
  ASSEMBLED FROM THE AUTHOR'S OWN PARTS, deliberately: the split section header
  his menus and occasions bands already use, his muted prose voice, and a plate
  in his own large radius. Drawing a NEW composition would mean new CSS in a
  stylesheet whose whole contract is that it is the author's file; the rules
  this band adds are a reading measure for prose and the frame, and they are in
  the appended tenant-state block with their reason beside them.
...
@endphp
    <section class="text-band section container" id="{{ $section->key }}" data-block="text" data-variant="garden-note">
