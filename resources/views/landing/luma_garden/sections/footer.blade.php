{{--
  The garden hub (data-block="footer", data-variant="garden-hub").
...
@php
    ...
    // instead — "Tue–Sat dinner · Wed–Sun lunch" — which is a claim only the
    // restaurant can make and which nothing on the record could derive; this
    // row states what the record actually knows.)
    $todayHours = null;
...
@endphp
  <footer class="site-footer" id="site-footer" data-block="footer" data-variant="garden-hub">
...
{{-- The same lockup rule as the header's — one logo, two places, read from the
     one resolved value, and the same derived TAIL emphasis this design's
     header lockup carries (see Copy::wordmark()). --}}
...
@if (filled($chatFrameUrl ?? null))
        ...
        <iframe class="ai-panel" id="garden-chat-panel" src="{{ $chatFrameUrl }}"
        ...
        <button class="ai-launcher" type="button"
                aria-controls="garden-chat-panel" aria-expanded="false"
...
@endif
    ...
    <div class="container footer-bottom">
      {{-- THE LEGAL LINE. The author's is "© 2026 Luma Garden. Fictional
