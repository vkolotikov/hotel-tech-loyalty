{{--
  Before your table (data-block="faq", data-variant="dining-questions").

  The author's split on the night-soft band: an intro column — mono eyebrow and
  display heading — and beside it a list of native <details>, each summary a
  question set in the display face. Native, so it works with no JavaScript at
  all.
  ...
  none is written here — the same reasoning that made kit 03's first-open a
  DERIVED property of the list rather than a per-pair leaf, pointed at a
  design that does not want it.

  THIS DESIGN DRAWS NO INTRO PARAGRAPH. His column is an eyebrow and a heading
  and nothing else, so `faq.subtext` is not read here and `content_fields`
  does not offer it.
...
@php
    ...
    $title = $heading !== '' ? $heading : ($kicker !== '' ? $kicker : __('Before the table'));
@endphp
    <section class="faq section" id="faq" data-block="faq" data-variant="dining-questions" aria-labelledby="faq-title">
...
@endif
          {{-- The accent companion belongs to the tenant's HEADING and only to
               ...
               default name, and appending "…in your accent colour at the end
               of the heading" to a fallback the tenant did not write would put
               the emphasis on somebody else's words. --}}
