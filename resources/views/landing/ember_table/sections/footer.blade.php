{{--
  The restaurant hub (data-block="footer", data-variant="restaurant-hub").
  ...
  WHAT IS RENDERED IS WHAT EXISTS. Every column here is gated:
    ...
    - the review link only where a review form is actually reachable — see
      ...
      endpoint asks. "Leave a review" pointing at "Form not found or inactive"
  ...
  THE HUB IS TOLD HOW MANY COLUMNS IT REALLY HAS. The author's five-track grid
  assumes all of them; `.footer-hub--1/2/3` (the stylesheet's appended
  tenant-state block) close the row up instead of leaving a gap. This hub is
  the same contract all six kits share — see the collection's own README.

  THE AI SLOT keeps its footprint whether or not a chat widget exists — it is
  what stops the fixed launcher covering the last line of the footer, which is
  the author's reason for reserving it.
--}}
@php
    ...
    // keeps whatever spacing the tenant typed. A + is meaningful only in first
    // position, so any later one is dropped rather than dialled.
    ...
    // footer — a week's ledger belongs on a contact page and this kit does not
    // have one. Today is the TENANT's today, not the server's: a timezone
    // typed into an admin field can be anything at all, so a bad one costs
    // this line and nothing else. (The author writes a service PATTERN here
    // instead — "Wed–Sat evenings · Fri–Sun lunch" — which is a claim only the
    // restaurant can make; his hero's `proof` leaf is where a tenant writes
    // exactly that, and this row states what the record actually knows.)
    $todayHours = null;
    ...
    // Content columns beside the always-present brand column: the review link,
    // the contact details and the social icons. The slot is not one of them —
    // it is the reserved corner, and the grid template names it separately.
    // FOUR is the author's own rule, unmodified.
    $hubColumns = 1 + ($showsReview ? 1 : 0) + ($showsChannel ? 1 : 0) + ($social !== [] ? 1 : 0);

    // The sentence after the copyright. The author writes one ("Fictional
    // demonstration."); with no leaf for it the conversion printed the © line
    // alone.
    ...
    // both words of his name, joined by a slash and set in the mono face —
    // where the other four kit templates draw a single display-face letter. It
    // is derived from the same $brandName chain and degrades to ONE letter for
    // a one-word business, which is what those four already draw.
    $footerInitial = collect(preg_split('/\s+/u', trim((string) $brandName), -1, PREG_SPLIT_NO_EMPTY) ?: [])
        ->take(2)
        ->map(fn ($word) => mb_strtoupper(mb_substr($word, 0, 1)))
        ->implode('/');
@endphp
  <footer class="site-footer" id="site-footer" data-block="footer" data-variant="restaurant-hub">
    <div class="container footer-hub footer-hub--{{ $hubColumns }}">
      <div class="footer-hub__booking">
@if (filled($brandName))
        <a class="brand brand--footer" href="#top" aria-label="{{ $brandName }}">
{{-- The same lockup rule as the header's — one logo, two places, read from the
     one resolved value, and the same derived infix emphasis on a conjunction
     in the business's own name (see Copy::wordmark()). --}}
...
@endif
@if ($bookingHref !== null)
        <a class="button button--cream" href="{{ $bookingHref }}"@if ($bookingIsFlow) data-action="open-booking" target="_blank" rel="noopener"@endif>@include('landing.shared.kit-icon', ['name' => 'calendar']){{ $chromeLabels['footer'] }}</a>
@endif
      </div>

@if ($showsReview)
      <div class="footer-hub__review" data-block="feedback" data-variant="footer-link">
        <p class="footer-label">{{ __('Diner review') }}</p>
...
@if ($email !== null)
        {{-- THE ONE CHANNEL THE AUTHOR LABELS RATHER THAN PRINTS. His line
             reads "Email the restaurant", not the address. The address stays
             the fallback, and the mailto: is the address either way, so
             nothing about where the link GOES depends on the wording. Its two
             ...
             it. --}}
        <a href="mailto:{{ $email }}">@include('landing.shared.kit-icon', ['name' => 'mail']){{ trim((string) ($contactCopy['email_label'] ?? '')) !== '' ? $contactCopy['email_label'] : $email }}</a>
@endif
@if ($todayHours !== null)
...
@if ($social !== [])
      {{-- The author's own nav, icon for icon. Each anchor's accessible name
           ...
           text — and rel="noopener" with target="_blank" because these are the
           page's only outbound links to somewhere a tenant typed. --}}
...
@if (filled($chatFrameUrl ?? null))
        {{-- hidden is load-bearing rather than a courtesy: [hidden] is
             ...
             request — costs nothing at all until someone presses the launcher.
             loading="lazy" says the same thing to the browser. --}}
        <iframe class="ai-panel" id="ember-chat-panel" src="{{ $chatFrameUrl }}"
        ...
        {{-- aria-expanded is maintained by landing/kit.js and is also what
             ...
             second class to fall out of step with the state screen readers are
             told about. --}}
        <button class="ai-launcher" type="button"
                aria-controls="ember-chat-panel" aria-expanded="false"
...
@endif
    ...
    <div class="container footer-bottom">
      {{-- THE LEGAL LINE. The author's is "© 2026 Ember Table. Fictional
           demonstration." — a registration or disclaimer sentence after the
           copyright, which had no leaf and so was dropped.
