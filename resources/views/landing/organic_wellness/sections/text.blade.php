{{--
  A tenant-added words band (data-block="text", data-variant="soft-note").
  ...
  header his rituals, mosaic and guest-notes bands already use, his ruled
  eyebrow, and an organic frame in the same radius and shadow as his story
  and team pictures. Drawing a NEW composition would mean new CSS in a
  stylesheet whose whole contract is that it is the author's file; the rules
  this band adds are a reading measure for prose and the frame, and they are
  in the appended tenant-state block with their reason beside them.
...
@php
    ...
    // picture for somebody who cannot see it; the caption is printed under
    // the frame in the author's muted voice. Neither is invented.
    $alt     = trim((string) ($fields['alt'] ?? ''));
    $caption = trim((string) ($fields['caption'] ?? ''));
@endphp
    <section class="section" id="{{ $section->key }}" data-block="text" data-variant="soft-note">
      <div class="container">
