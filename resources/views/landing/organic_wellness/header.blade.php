{{--
  The header (data-block="header", data-variant="calm-sticky").
  ...
  word for what it is, and the links are the bands this page is actually
  going to render. There is no `content.header.*` and there should not be — a
  nav a tenant can type into is a nav that can point at a band that is not
  there.

  THE WORDMARK'S CONJUNCTION IS THE ONE INFIX EMPHASIS IN THESE KITS. The
  author writes `Morrow <em>&amp;</em> Moss` here and again in his footer,
  with the ampersand set in the accent colour and the display italic. It is
  NOT a heading and NOT a leaf — it is derived from the business's own name,
  which is chrome — so App\Landing\Copy::wordmark() draws it, out of escaped
  fragments, from the name itself: a single `&` standing alone between two
  words. A business with no such ampersand gets exactly the lockup it always
  had. See that method's own note for why a companion leaf could not have
  expressed this one.
...
@php
    ...
    // mark is set in caps by the stylesheet's own font rather than by CSS
...
@endphp
  <header class="site-header" data-block="header" data-variant="calm-sticky">
...
{{-- THE LOGO, IF THE BUSINESS HAS ONE (template fidelity 4.6). It takes the
     monogram's own organic square, so the lockup does not move; the monogram
     is the fallback it always was. aria-hidden and alt="" on both: the
     wordmark beside them already names the business, and the anchor carries
     its own aria-label — a logo read out as well would announce the name
     twice. --}}
...
@if ($bookingHref !== null)
      <a class="button button--primary button--small header-booking" href="{{ $bookingHref }}"@if ($bookingIsFlow) data-action="open-booking" target="_blank" rel="noopener"@endif>@include('landing.shared.kit-icon', ['name' => 'calendar']){{ $chromeLabels['header'] }}</a>
@endif
...
@endforeach
@if ($bookingHref !== null)
          <a class="button button--primary" href="{{ $bookingHref }}"@if ($bookingIsFlow) data-action="open-booking" target="_blank" rel="noopener"@endif>@include('landing.shared.kit-icon', ['name' => 'calendar']){{ $chromeLabels['mobile'] }}</a>
@endif
        </nav>
      </details>
@endif
