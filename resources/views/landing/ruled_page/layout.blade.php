...
{{--
  ...
  aggregateRating and review are gated on the exact same switch
  sections/reviews.blade.php already renders its own aggregate behind:
  $content->reviewStats is null below PageContent::MIN_REVIEWS_FOR_AGGREGATE
  (4) ratings, org-wide. A single five-star testimonial is not a rating,
  and structured data claiming one would be the fabrication that partial's
  own docblock already refuses to commit on the visible page — so below
  four, this block publishes neither. review's author name and reviewBody
  truncation match that same partial exactly, so structured data never
  claims to quote more of a review than the page actually shows.
--}}
@php
...
@foreach ($sections as $section)
  ...
  {{-- A section key with no partial is skipped rather than fatal. Section
       rows are stored data and the partials are shipped code; the two can be
       out of step across a deploy or a template rollback, and a live customer
       page losing one band is recoverable where a 500 is not. has() above has
       already reduced $section->key to the keys this template knows, so this
       cannot hide a typo — only a partial that is genuinely not there yet. --}}
  @continue(! view()->exists('landing.ruled_page.sections.' . $section->key))
