...
/**
 * Tool registry + executor for CrmAiService.
 *
 * Holds the Anthropic tool schemas and the implementations the model can call.
 * Extracted from CrmAiService so the service itself stays focused on chat
 * orchestration (system prompt, HTTP loop, message shaping). All tools are
 * self-contained — they hit Eloquent / config / other services directly and
 * carry no per-conversation state, so they live equally well on a separate
 * class instantiated per request.
 */
class CrmAiToolset
{
    /* ────────── Tool Definitions ────────── */
    ...
    public function executeTool(string $name, array $in): array
    {
        try {
            return match ($name) {
