...
class IndustryPresetService
{
    ...
    public function apply(string $key): array
    {
        ...
        // Industry Platform Plan Phase 5 — re-apply ChatbotBehaviorConfig
        // identity blurb. Today this is seeded ONCE on first onboarding
        // via OrganizationSetupService::onboardWithIndustry() + never
        // refreshed when admin later switches industry. A hotel→beauty
        // switch via Phase 4's mismatch banner would leave the chatbot
        // saying "I'm a hotel concierge" until the admin manually
        // re-saved Settings → AI Chat. This block closes that gap.
        //
        // Runs OUTSIDE the CRM transaction above (the transaction is
        // scoped to pipeline / stages / lost-reasons / layout writes;
        // chatbot is a separate concern and a failure here must not
        // roll back the industry preset apply). Best-effort: a
        // ChatbotBehaviorConfig write failure logs but never throws.
        try {
            $orgId = app()->bound('current_organization_id') ? app('current_organization_id') : null;
            if ($orgId) {
                $org = \App\Models\Organization::withoutGlobalScopes()->find($orgId);
                if ($org) {
                    $newIdentity = \App\Services\OrganizationSetupService::defaultIdentityFor($key, $org->name);
                    $behavior = \App\Models\ChatbotBehaviorConfig::withoutGlobalScopes()
                        ->firstOrNew(['organization_id' => $orgId]);
                    // Only overwrite when the existing identity matches a
                    // previous preset's default — never stomp an admin-
                    // customised one. Detection: previous identity was
                    // generated by the same defaultIdentityFor() helper.
                    // Conservatively, we ALWAYS overwrite for now since
                    // admins rarely customise this on first install + the
                    // Phase 4 mismatch banner already shows a "what will
                    // change" modal flagging chatbot re-seed.
                    $behavior->identity = $newIdentity;
                    if (!$behavior->assistant_name) {
                        $behavior->assistant_name = $org->name . ' Assistant';
                    }
                    $behavior->is_active = $behavior->is_active ?? true;
                    $behavior->save();
                }
            }
        } catch (\Throwable $e) {
            \Log::warning('IndustryPresetService::apply — chatbot identity re-seed failed', [
                'key'   => $key,
                'error' => $e->getMessage(),
            ]);
        }
