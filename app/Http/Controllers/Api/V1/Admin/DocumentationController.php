        ...
        return [
            [
                ...
                'articles' => [
                    ...
                    [
                        'title' => 'Growth & Enterprise features (not on Starter)',
                        'content' => "Five additional features are gated to the Growth and Enterprise plans (Starter excluded):\n\n**Email Campaigns** (`/campaigns`) — block-builder email broadcasts with variable substitution, send history, per-campaign analytics, send-test, duplicate-as-draft. On Starter the sidebar Campaigns item is greyed; admin can't create, edit, send, or duplicate campaigns. Existing scheduled sends (if any) keep going out via cron — the gate is on the admin surface only.\n\n**Reviews & Feedback** — review forms (Basic threshold-redirect + Custom questionnaire), submissions tracking, integrations (Google / TripAdvisor / Trustpilot), post-stay invitation cron + funnel dashboard. Public submission URLs (`/review-form/{key}`) stay open even on Starter so existing forms keep accepting submissions; only the admin Reviews dashboard at Marketing → Reviews is gated.\n\n**Live Chat Inbox & Lead Tracking (Engagement Hub)** — unified `/engagement` page replacing the legacy Inbox + Visitors split. Smart-priority sort, hot-lead detection, intent tagging, AI brief drawer, daily summary email opt-in, fullscreen monitor view. On Starter the sidebar Engagement item is greyed. The legacy detail endpoints stay open so historical conversations remain readable if the org upgrades again.\n\n**Digital Wallet Cards (Apple + Google)** — Settings → Wallet passes config (.p12 cert upload, WWDR cert, Google service account). On Starter, admin can't upload new credentials or update existing ones. The member-facing pass generation endpoint stays open so existing members with cards already added to their wallet keep receiving tier + points updates.\n\n**Website Chatbot (AI)** — `/chatbot-setup` covering behavior config, model config (OpenAI), knowledge base CRUD, popup automation rules, AI training/fine-tuning, test-chat / probe-model. On Starter, admin can't configure or train the chatbot. The public widget on the customer's website keeps serving from the LAST-SAVED config — visitors keep getting AI responses, the customer just loses the ability to retune it.",
                    ],
                    [
                        'title' => 'Why is X greyed out in the sidebar?',
                        'content' => "When a sidebar item appears greyed with a small gold lock badge, the organization's current plan does not include that feature. The most common cases:\n\n**Enterprise-only (Starter + Growth see lock):**\n- **Planner (greyed)** — Time Management Platform.\n- **Brands (greyed)** — Multi-brand portfolios.\n- **Staff AI Copilot — floating button missing** — no greyed state because a floating action button reads as broken UI when greyed.\n\n**Growth+ (Starter sees lock):**\n- **Engagement (greyed)** — Live chat inbox + lead tracking.\n- **Campaigns (greyed)** — Email campaigns.\n- **Chatbot Setup (greyed)** — Website chatbot admin config.\n- **Reviews tab inside Marketing hub** — the hub itself stays accessible (it has other tabs too: Notifications, Email Templates) but the Reviews tab content shows the upgrade prompt on Starter.\n- **Settings → Wallet passes** — Wallet config tab in Settings.\n- **AI Insights (greyed on Starter)** — AI-generated weekly insights.\n\nClick any greyed item to open the upgrade modal with a one-click Upgrade button. The plan + the gated feature flow through to the modal so staff see exactly which plan unlocks the feature they were trying to use.\n\nThe gates are advertised on the Login/Register pricing card and on Settings → Billing & Subscription.",
                    ],
                    [
            ...
            [
                ...
                'icon' => 'Users',
                'description' => 'Manage contacts, the sales pipeline (Leads to Deals), reservations, corporate accounts, and daily operations. Records open as in-place drawers; exports are styled Excel workbooks.',
                'articles' => [
                    [
...
**Best Practice:** Set a follow-up task on every open inquiry. Use the AI Smart Panel's win-probability + suggested action to prioritise outreach.",
                    ...
                    [
                        ...
                        'content' => "Settings → Pipelines is the central CRM config page. **Five collapsible accordions**, with only the most-used one (Fields) expanded by default — the rest stay folded so the page isn't overwhelming.\n\n**1. Quick setup by industry** — One click reshapes the whole CRM (pipeline stages + lost reasons + per-entity field layouts + custom fields) for your vertical. 8 presets: Hotel, Beauty/Spa, Medical/Healthcare, Legal, Real Estate, Education, Fitness, Restaurant. Switching is safe — existing inquiries migrate to the matching new stage by kind, lost-reason history is preserved, custom-field values stay on entity rows even when the field schema is removed.\n\n**2. Fields** — The unified Field Manager. 6 tabs (Leads / Deals / Customers / Companies / Members / Tasks) each control which fields show on their list page + (for most entities) their detail page. See the dedicated \"Field Manager\" doc section for the full list of 89 toggles.\n\n**3. Custom fields** — Define your own fields per entity (Leads, Guests, Companies, Tasks). 10 field types: text, multi-line, number, date, select, multi-select, checkbox, email, phone, URL. Quick-add chips (Birthday, Allergies, Tags…) and a visual type picker with examples make it easy for non-technical users. Custom fields can be promoted to extra columns in the leads list via the \"List?\" toggle on each field row.\n\n**4. Pipelines & stages** — Multi-pipeline support. Each pipeline is a named sequence of stages. Most orgs need one (\"Sales\"); hotel groups running MICE / corporate sales add a second. Per-stage editor: rename, recolor, change kind (open/won/lost), set default win-probability. Move up/down arrows for reordering. Server enforces \"always one open / won / lost stage\" so the won/lost flow always has a target.\n\n**5. Lost reasons** — Taxonomy used by the Lost modal's reason picker. In-use reasons soft-deactivate instead of hard-deleting so the funnel report keeps historical labels.",
                    ],
                    [
                        'title' => 'Field Manager — what each tab controls',
                        'content' => "Settings → Pipelines → **Fields** is where you control which information appears across every CRM list + detail page. **One unified panel with 6 entity tabs**, each showing an on/off count chip so you can see at a glance how many fields are visible vs hidden.\n\nEach tab has bulk \"All on / All off\" buttons per section and a search box (useful for long lists). Saved per organization — no per-user views yet. Changes save with a single click and apply globally to every admin on your team.\n\n**Always-shown** across every entity: the identity column (name + avatar), status pills that block work decisions, and the actions column (edit / open / kebab). Those can't be hidden — turning everything off would break the page.\n\n**Leads (Inquiries)** — 13 Add-Inquiry-form toggles (Check-in / Check-out / Rooms / Inquiry type / Source / Room type / Rate / Total value / Status / Priority / Assigned to / Special requests / Notes), 6 list-column toggles (Stay / Value / Owner / Touches / Next task / Bulk-select), and 10 Inquiry-detail-page toggles (Contact / Stay block / Special requests / Pipeline block / Linked reservation / Custom fields / Open guest profile link / AI Smart Panel / Open tasks / Attachments). When all three right-column panels (AI / Tasks / Attachments) are hidden the column collapses entirely — no empty 300px gap.\n\n**Deals** — 7 list-column toggles (Product details / Amount / Payment / Fulfillment / Next action / Due date / Owner). Deals share the same detail page as Inquiries, so their detail-page toggles live on the Leads tab.\n\n**Customers** — 5 list-column toggles (Contact / Company / Activity / VIP badge / Position title) and 11 detail-page toggles (Header pills / Stats strip / B2B profile / Location / Hotel preferences / Activity dates / Tags / Notes / Activity log / Recent reservations / Recent inquiries). The customer-list row uses a dynamic CSS grid so hiding a column collapses its space cleanly instead of leaving a gap.\n\n**Companies** — 8 list-column toggles (Industry / Contact person / Account manager / Contract / Rate / Discount / Revenue / Status) and 8 detail-page toggles (Vitals strip / Renewal-soon banner / Billing fields / Billing address / Notes / Custom fields / Linked deals / Recent reservations).\n\n**Members** — 7 list-page toggles including a KPI strip toggle (KPI strip / Phone / Source / Tier / Points / Joined / Status) and 8 detail-page toggles (Hero stats row / AI Analysis button / Transactions tab / Journey tab / Settings tab / Recent activity card / Member QR card / Award/Redeem panel). Tab visibility is enforced on both the tab bar AND the tab content, so a stale `?tab=…` URL can't render a hidden section.\n\n**Tasks** — 6 task-card toggles (Description / Type label / Due date / Assignee / Inquiry link / Outcome). The colored type-icon next to the checkbox always stays so the visual hierarchy survives even when the uppercase label chip is off.\n\n**Industry presets are opinionated.** Hotel + Real Estate default to \"everything visible\". Beauty / Medical / Legal / Fitness / Restaurant / Education auto-hide the stay-centric bits (Stay block, Hotel preferences, Linked reservation, Recent reservations) and the hotel-only Companies columns (Contract / Rate / Discount). Admin can flip any of them back on per-org.",
                    ],
                    [
            ...
            [
                'slug' => 'reviews',
                'title' => 'Reviews & Surveys',
                'icon' => 'Star',
                'description' => "Build feedback surveys for websites, email and on-site tablet kiosks. Compose your own questions with branching follow-up logic, style each survey, push different surveys to different devices, funnel happy guests to public review sites, and read per-survey analytics.",
                'articles' => [
                    [
                        'title' => "Surveys vs the classic rating form",
                        'content' => "The Reviews module has two kinds of form:\n\n**Basic survey** — a fixed 5-star rating plus an optional comment box. Its whole purpose is the *threshold redirect*: set a redirect threshold (e.g. 4★) and guests who rate at or above it see buttons to your Google / TripAdvisor / Trustpilot page on the thank-you screen. Ratings below the threshold stop at the thank-you message so negative feedback stays private. A basic survey has no question editor — its settings ARE its build surface (comment toggle, threshold, redirect prompt, intro/thank-you text).\n\n**Custom survey** — you compose the questions yourself in the visual builder. Use this for anything beyond a single rating: NPS, smileys, multi-question questionnaires, branching follow-ups. Create one from Reviews → Forms → New.\n\nBoth live under **Marketing → Reviews & Surveys**, which has five tabs: Submissions, Invitations, Forms, Devices, Integrations. Each org can have many surveys; one per type is the default used for automatic post-stay invitations.",
                    ],
                    [
                        'title' => "The question builder",
                        'content' => "Open a custom survey and the builder has three tabs: **Questions**, **Design**, **Analytics**.\n\nOn the Questions tab, **Add question** opens a visual gallery — every type shows an icon, a plain-language description and a mini example of how it looks to the guest:\n\n- **Star Rating** — classic 1–5 stars\n- **Smileys** — tap a face; the fastest input on a kiosk. Quick preset sets: 5 smileys / 3 smileys / 👍👎 thumbs / hearts, or fully custom emoji + labels\n- **NPS (0–10)** — the \"would you recommend us\" question, with the red / amber / green strip and editable end labels (e.g. \"Not likely at all\" → \"Extremely likely\")\n- **Scale (1–10)** — numeric satisfaction scale, also with optional end labels\n- **Single Choice** / **Multiple Choice** — add / remove / reorder options\n- **Yes / No** — one quick decision\n- **Short Text** / **Long Text** — open feedback\n\nDrag the grip handle to reorder. Toggle **Required** per question. Each card shows a live inline preview. Deleting a question automatically heals any follow-up logic that pointed at it.",
                    ],
                    [
                        'title' => "Branching & follow-up logic",
                        'content' => "Any question can be shown **Always** or **Only if…** an earlier answer matches — this is how you ask \"what went wrong?\" only after a low score.\n\nOn each question card, the **Show this question** control switches between *Always* and *Only if…*. When you pick *Only if…*:\n\n1. Choose which earlier question it depends on (listed by name + type, not just \"Q3\").\n2. Pick an operator — and the operators adapt to that question's type. A star rating offers *is at most / at least / exactly*; choices offer *is / is not / is one of*; multi-choice offers *includes*; yes/no just picks Yes or No. No nonsensical combinations.\n3. Set the trigger value by **tapping the parent's own answer widget** — tap 2 stars, tap the \"6\" pill on the NPS strip, tap the choice chips (emoji shown) — instead of typing raw values.\n\n**One-click follow-ups:** every rateable / choice question has an **Add follow-up** button that inserts a pre-wired child right beneath it — e.g. stars ≤ 2 → \"What could we improve?\", NPS ≤ 6 → \"What would it take to score us higher?\". The parent shows a \"N follow-ups\" badge and the child shows a branch connector quoting the rule in plain words.\n\nIn the live survey, follow-ups appear the instant the trigger answer is chosen and are skipped otherwise — and answers to skipped questions are never stored.",
                    ],
                    [
                        'title' => "Design & themes",
                        'content' => "The **Design** tab restyles a survey without touching its questions. Everything here updates the phone-framed **live preview** on the right instantly (question changes need a Save + Reload).\n\n**Layout** — *Classic card* (all questions on one scrolling page — best for email links) or *Full-screen survey* (one question per screen, big touch targets, tap to auto-advance — best for kiosks and website popups). Kiosk devices always render full-screen regardless of this setting.\n\n**Theme** — five gradient presets (Ocean, Aurora, Sunset, Forest, Midnight) or Custom with your own gradient / button / text colors (swatch + hex). Add a logo URL shown at the top.\n\n**Welcome screen** — optional intro screen before the first question (kiosks always show one). Set the title, subtitle and start-button label.\n\n**Thank-you screen & kiosk timing** — custom thank-you title + message, and two sliders that only apply to kiosks: *reset after thanks* (return to the welcome screen N seconds after a guest finishes) and *idle reset* (reset if a guest walks away mid-survey).",
                    ],
                    [
                        'title' => "Delivery channels",
                        'content' => "One survey can go out through every channel at once — each response is tagged with where it came from for analytics.\n\n**Web link** — every survey has a rotatable embed key. The public URL is `{app_url}/review/{form_id}?key={embed_key}`. Share it anywhere, print it, or QR it.\n\n**Website widget** — the Design tab generates a copy-paste `<script>` snippet with three modes: a floating side **button** (always available), an auto-opening **popup** (centered, after a delay), or a corner **slide-up** card. It's polite: after a visitor submits, the survey stays away for 90 days; after they dismiss an auto-open, 7 days. A one-line **iframe** snippet is offered as an alternative for inline embedding.\n\n**Email invitations** — send a one-time tokenized link `{app_url}/review/t/{token}` that tracks open and submit events in the funnel (see Sending invitations).\n\n**Kiosk devices** — registered tablets that show whichever survey you assign them (see Feedback kiosks).\n\nRotating the embed key invalidates existing shared links + widget snippets (not invitation tokens) — use only if a key leaked. Re-copy the snippet from the builder afterward.",
                    ],
                    [
                        'title' => "Feedback kiosks (tablet devices)",
                        'content' => "**Reviews → Devices** turns any tablet into a self-serve feedback station.\n\n1. **Register** the tablet (name + optional location, e.g. \"Reception iPad\"). It gets a stable kiosk link `{app_url}/k/{device_key}`.\n2. Open that link on the tablet — tap the **QR** button on the device row, scan it with the tablet camera, open, then add to the home screen / enable the browser's kiosk mode so it stays full-screen.\n3. **Assign a survey** from the dropdown on the device row.\n\nThe magic: the tablet checks its assignment every 60 seconds, so **reassigning or restyling a survey from the admin repoints the tablet within a minute — you never touch the device again**. Different kiosks can show different surveys. A green dot on each row shows which kiosks are live right now (heartbeat in the last 2 minutes).\n\nKiosk surveys always render full-screen, auto-advance on tap, show a welcome screen, auto-reset after the thank-you screen and after idle abandonment, and never show external-review redirects (those only make sense on a guest's own phone). Rotate a device's key to kill a lost tablet's link.",
                    ],
                    [
                        'title' => "Sending email invitations",
                        'content' => "Three ways to send a survey invitation:\n\n**1. One-off from a profile** — Member, Guest and Booking detail pages each have a *Send review* button. Pick the survey, optionally override the subject.\n\n**2. Ad-hoc email** — Reviews → Invitations → send to any email address (useful for contacts without a profile).\n\n**3. Automatic post-stay** — on the survey's Design tab, turn on **Post-stay automation** and set the days-after-checkout delay. A daily 09:00 job invites every booking-engine guest (with an email) who checked out that many days earlier, skips cancelled bookings, and invites each booking only once. **Enable this on ONE survey only**, or guests get multiple emails. Ops can dry-run it with `php artisan reviews:send-post-stay --dry-run`.",
                    ],
                    [
                        'title' => "Analytics & submissions",
                        'content' => "**Per-survey analytics** live on the builder's **Analytics** tab (7 / 30 / 90-day window):\n\n- KPI row: views, responses, completion rate, average rating, NPS\n- Response trend (views vs responses over time)\n- **By channel** and **by kiosk device** splits — see whether the tablet, the website widget, email or a direct link is driving feedback\n- **Per-question breakdowns** — distribution bars + averages for scale/rating/smiley/NPS questions, option counts for choices, and the latest free-text answers verbatim\n\n**Reviews → Submissions** lists every response with an **Export CSV** button that honors all active filters (survey, rating, redirect state, date range); UTF-8 with BOM for Excel. **Reviews → Invitations** shows the delivery funnel: Sent → Opened → Submitted → Redirected (or Failed).\n\nThe module-level KPI strip at the top of the Reviews page shows org-wide average rating, NPS, submit rate and external redirects over the last 30 days.",
                    ],
                    [
                        'title' => "iframe events (for your own site)",
                        'content' => "The public survey page sets `X-Frame-Options: ALLOWALL` and `frame-ancestors *`, so any site can embed it. Inside an iframe it emits `window.postMessage` events the parent page can listen for:\n\n- `{ source: 'hotel-tech-review', event: 'review-loaded', form_id, form_type }` — on mount\n- `{ source: 'hotel-tech-review', event: 'review-submitted', submission_id, rating }` — on submit\n- `{ source: 'hotel-tech-review', event: 'review-redirected', submission_id, platform }` — when a guest clicks through to Google / TripAdvisor / Trustpilot\n\nUse these to trigger your own analytics, a thank-you modal, or a booking handoff. (The bundled website widget already listens for `review-submitted` to handle its own frequency capping.)",
                    ],
                ],
            ],
            [
                ...
                'articles' => [
                    ...
                    [
                        'title' => 'AI Usage & Plan Limits',
                        'content' => "Settings → **AI Usage** shows month-to-date AI spend for your organization, broken down by model and by feature. Every AI call across the platform (admin chat, website chatbot, member chat, smart-panel briefs, sentiment, upsell, etc.) is recorded in a per-org ledger.\n\n**KPI strip** — spent this month, plan-cap status, total AI calls + tokens, allowed models.\n\n**Budget status** has three states:\n- **Healthy** — under 80% of your plan's monthly AI cap\n- **80%+** — amber warning banner, you'll hit the cap soon\n- **Over plan** — red banner. AI features keep working today (v1 is track-only) but contact billing for a higher cap.\n- **Unlimited** — your plan has no AI cap configured.\n\n**By model / By feature** lists where the dollars go. \"By feature\" is the more useful view for cost optimization — if `website_chatbot` dominates your bill but conversions are low, the knob to turn is the chatbot's model selection or the maximum tokens per reply.\n\n**Plan models** — if your plan restricts which AI models you can use, only those models will work. Calls to other models fail with a clear \"model not included in plan\" error. Upgrade your plan to unlock more capable (and more expensive) models like Claude Opus or GPT-5.",
                    ],
                ],
            ],
            [
    ...
    public static function getFaq(): array
    {
        return [
            ...
            [
                'category' => 'Plans & Pricing',
                'question' => 'Why are Email Campaigns / Reviews / Engagement Hub greyed out?',
                'answer' => 'These features are included on the Growth and Enterprise plans (not Starter). Click the greyed sidebar item to see the upgrade modal with a one-click Upgrade button. If you\'re on Growth and they\'re still greyed, the entitlement cache may not have synced yet — try refreshing once or check Settings → Billing & Subscription to confirm your current plan.',
            ],
            [
                'category' => 'Plans & Pricing',
                'question' => 'I downgraded my plan — what happens to my existing data?',
                'answer' => 'Data is never deleted on downgrade. Existing review forms keep accepting public submissions. Existing chatbot keeps serving the customer\'s site from the last-saved config. Existing email campaigns + drafts remain in the database. Member wallet cards already issued keep receiving updates. You just lose the ability to MANAGE these surfaces from the admin until you upgrade again. Re-upgrading restores immediate access to everything as it was.',
            ],
            [
                'category' => 'Plans & Pricing',
                'question' => 'Will the public chatbot disappear from my website if I downgrade?',
                'answer' => 'No. The public widget keeps serving from the last-saved chatbot configuration, knowledge base, and popup rules. Visitors on the customer\'s website keep getting the same AI responses they did before. You just can\'t reconfigure it from the admin Chatbot Setup page — the existing configuration is frozen until you upgrade again.',
            ],
            [
                'category' => 'General',
            ...
            [
                'category' => 'Reviews',
                'question' => 'How do I set up a feedback tablet (kiosk)?',
                'answer' => 'Go to Marketing > Reviews & Surveys > Devices. Register the tablet (give it a name), then tap the QR button on its row and scan it with the tablet\'s camera to open the kiosk page - add it to the home screen or enable your browser\'s kiosk mode so it stays full-screen. Finally pick which survey it shows from the dropdown on that row. The tablet re-checks its assignment every minute, so you can change or restyle its survey from the admin without ever touching the device again.',
            ],
            [
                'category' => 'Reviews',
                'question' => 'How do I add a survey to my website?',
                'answer' => 'Open the custom survey, go to the Design tab, scroll to Website widget & embed. Choose a mode - floating side button, auto-open popup, or corner slide-up - set the label/color/delay, and copy the generated script tag into your site before the closing body tag. There is also a one-line iframe snippet for embedding a survey inline in a page. Auto-open modes are polite: a visitor who submits is not asked again for 90 days, and who dismisses is not for 7.',
            ],
            [
                'category' => 'Reviews',
                'question' => 'How do I make a follow-up question that only shows for bad ratings?',
                'answer' => 'In the custom survey builder, click Add follow-up on the rating question - it inserts a pre-wired question below it (e.g. stars 2 or lower to \'What could we improve?\'). Or on any question set \'Show this question\' to \'Only if...\', pick the earlier question, choose an operator (is at most / is / is one of...), and tap the trigger value on the parent\'s own widget. Follow-ups appear live the moment the guest gives the matching answer, and are skipped otherwise.',
            ],
            [
                'category' => 'Reviews',
                'question' => 'Can I change the look of a survey (colors, kiosk screens)?',
                'answer' => 'Yes - the Design tab. Pick a gradient theme (Ocean, Aurora, Sunset, Forest, Midnight) or custom colors, add your logo, and write the welcome and thank-you screens. Choose Classic card (email links) or Full-screen (kiosks/popups). Everything updates live in the phone preview as you edit; click Save design when done.',
            ],
            [
                'category' => 'CRM',
                'question' => 'Why don\'t I see confirmed or lost leads in the Leads list?',
                'answer' => 'By design - Leads shows only active work. When you mark a lead Confirmed it becomes a Deal (find it under Deals). When you mark it Lost it leaves Leads but the person stays in Contacts > Customers (closing a lead never deletes anyone). So Leads is your live pipeline, Deals is won work in fulfilment, and Customers is everyone you have dealt with.',
            ],
            [
                'category' => 'CRM',
                'question' => 'Why does clicking a lead/deal/customer open a panel instead of a new page?',
                'answer' => 'That is intentional - records now open as a right-side drawer so you keep your list, filters and scroll position. You can edit the profile, see activity and change status right there. The full detail page is still one click away from inside the drawer.',
            ],
            [
                'category' => 'CRM',
